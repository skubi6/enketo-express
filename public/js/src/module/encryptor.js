...
var BASE64_ENCRYPTED_SYMMETRIC_KEY = encryptSymmetricKey( SYMMETRIC_KEY, PUBLIC_ASYMMETRIC_KEY );
...
function decryptSymmetricKey( base64EncryptedKey, privateKeyPem ) {
    ...
    var decryptedKey = privateKey.decrypt( encryptedKey, ASYMMETRIC_ALGORITHM, {
        md: forge.md.sha256.create()
    } );

    console.debug( 'decrypted symmetric key', decryptedKey );
    return decryptedKey;
}

function encryptSymmetricKey( symmetricKey, publicAsymmetricKey ) {
    var publicKeyPem = '-----BEGIN PUBLIC KEY-----' + publicAsymmetricKey + '-----END PUBLIC KEY-----';
...
function _getBase64EncryptedElementSignature() {}
